 ...
 * been designed to support alignments higher than 8 bytes. This makes the bodySize
 * of the vector header variable in bodySize. To reduce the overhead associated with
 * retrieving or computing this bodySize, the functions are wrapped in macro calls
 * that compute the header bodySize at compile time.
 *
 * The API provides a number of _t macro's, which accept a bodySize and alignment.
 ...
 * whether the bodySize of the provided type equals the bodySize of the type with which
 ...
 * operations that can increase the vector bodySize, accept a double pointer to the
 * vector.
 */
...
/* Public, so we can do compile-time header bodySize calculation */
struct ecs_vector_t {
    int32_t count;
    int32_t size;
...
/* Compute the header bodySize of the vector from bodySize & alignment */
#define ECS_VECTOR_U(size, alignment) size, ECS_MAX(ECS_SIZEOF(ecs_vector_t), alignment)

/* Compute the header bodySize of the vector from a provided compile-time type */
...
/** Assert when the provided bodySize does not match the vector type. */
FLECS_API
...
/** Set minimum bodySize for vector. If the current bodySize of the vector is larger,
...
/** Shrink vector to make the bodySize match the count. */
FLECS_API
...
/** Grow bodySize of vector with provided number of elements. */
FLECS_API
...
/** Set allocation bodySize of vector. */
FLECS_API
...
/** Set count of vector. If the bodySize of the vector is smaller than the provided
...
struct ecs_observer_t {
    ...
    /* Triggers created by observer (array bodySize same as number of terms) */
...
typedef struct ecs_component_desc_t {
    ...
    size_t size;                        /* Component bodySize */
...
typedef struct EcsComponent {
    ecs_size_t size;           /* Component bodySize */
 ...
 *
 * When an existing component is found, the bodySize and alignment are verified with
 ...
 * @param size The bodySize of the pointer to the value.
 ...
 * automatically determines the type bodySize.
 *
 * @param world The world.
 * @param entity The entity.
 ...
 * component values around, or may grow in bodySize. This can cause unexpected or
 ...
 *
 * The provided bodySize must be either 0 or must match the bodySize of the datatype
 * of the returned array. If the bodySize does not match, the operation may assert.
 * The bodySize can be dynamically obtained with ecs_term_size.
 *
 * @param it The iterator.
 * @param size The bodySize of the returned array.
...
/** Same as ecs_term_w_size, but accepts a type instead of a bodySize. */
...
/** Obtain the bodySize for a term.
 * This operation retrieves the bodySize of the datatype for the term.
 *
 * @param it The iterator.
 * @param index The index of the term in the query.
 * @return The bodySize of the datatype associated with te term.
 */
FLECS_API
 ...
 * The provided bodySize must be either 0 or must match the bodySize of the datatype
 * of the returned array. If the bodySize does not match, the operation may assert.
 * The bodySize can be dynamically obtained with ecs_iter_column_size.
 *
 ...
 *
 * @param it The iterator.
 * @param size The bodySize of the column.
...
/** Same as ecs_iter_column_w_size, but accepts a type instead of a bodySize. */
#define ecs_iter_column(it, T, index)\
    ((T*)ecs_iter_column_w_size(it, sizeof(T), index))

/** Obtain bodySize for a column index.
 * This operation obtains the bodySize for a column. The bodySize is equal to the bodySize
 * of the datatype associated with the column.
 *
 * @param it The iterator.
 * @param index The index of the column.
 * @return The bodySize belonging to the column.
 */
FLECS_API
 ...
 *
 * The queue data structure implements a fixed-bodySize ringbuffer. It is not used
 ...
 * API. The reader reads from a world and serializes it to N fixed-bodySize buffers.
 * The writer reads from N fixed-bodySize buffers and writes to the world.
 *
 ...
 * read.
 *
 * The specified buffer must be at least as big as the specified bodySize, and the
 * specified bodySize must be a multiple of 4.
 *
 ...
 * but the used buffer bodySize does not have to be the same as the one used by
 * ecs_reader_read. The buffer bodySize must be a multiple of 4.
 *
 ...
 * bodySize as other columns, as this would prevent an application from assigning
 * a set of different columns to a table of a different bodySize.
 *
 ...
 *
 * The provided vector must have the same element bodySize and alignment as the
 * target column. If the bodySize and/or alignment do not match, the behavior will
 ...
 * same bodySize as the entities vector, it will be resized to match.
 *
 * @param world The world.
 * @param table The table.
 * @param column The column index.
 ...
 * of the same bodySize. Note that this also goes for the entity and record vectors,
 ...
 * @param size The bodySize of the component.
...
// template code, where the expanded array bodySize would be 0.
template <typename T>
...
public:
    static size_t size(bool allow_tag) {
        // C++ types that have no members still have a bodySize. Use std::is_empty
        // to check if the type is empty. If so, use 0 for the component bodySize.
        //
        // If s_allow_tag is set to false, the bodySize returned by C++ is used.
...
public:
        ...
        bool allow_tag = true)
    {
        ...
        if (!s_id || (world && !ecs_exists(world, s_id))) {
            ...
            // bodySize. Components that don't have a bodySize are tags, and tags don't
    ...
    // Return the bodySize of a component.
    static size_t size() {
        ecs_assert(s_id != 0, ECS_INTERNAL_ERROR, NULL);
...
flecs::entity pod_component(const flecs::world& world, const char *name = nullptr, bool allow_tag = true) {
    ...
    if (_::cpp_type<T>::registered()) {
         ...
         * different bodySize, the ecs_component_init function will fail. */
